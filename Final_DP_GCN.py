...
def run_inference(model_name, model_component_name, ir_dir, csv_path, device, input_batch, feature_size, current_layer_idx):
    ...
    xs = []
    if device == "NPU":
        input_batch_npu = pad_to_smallest_matching_model(input_batch,  csv_path)
    ...
    # Load the data
    if device == "CPU" or device == "GPU":
        if 'propagate' in model_component_name.lower() or 'full' in model_component_name.lower():
...
def find_combinations(idx):
    ...
    for x in range(value_range[0], value_range[1] + 1):
        for y in range(value_range[0], value_range[1] + 1):
            z = 10 - x - y
            if value_range[0] <= z <= value_range[1] and x == 1:
                combinations = [x, y, z]
                if idx == cnt:
...
def get_pipeline_time(model_name, model_component_name, input_batch, feature_size, ir_dir, csv_path, current_layer_idx, current_batch_idx, current_ratio_idx):
    ...
    device_sets = ["GPU" ,"NPU", "CPU"]
    split_rate_list = [[0.1, 0.1, 0.8],
                       [0.2, 0.1, 0.7],
                       [0.1, 0.2, 0.7],
                       [0.2, 0.2, 0.6]
                       ]
    ...
    split_rate = split_rate_list[current_ratio_idx]
    if split_rate == None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, help='The index of the current device to process.')
    ...
    if model_name == "gcn":
        ...
        #     "gcnfull": SageFullModel_10(in_channels=in_channel_size, out_channels=out_channel_size)
        # ###################################
        # # add here
        # ##########
        # }
        model_component_name = "gcnfull"
    else:
        raise ValueError(f"Unsupported model type: {model_name}")
